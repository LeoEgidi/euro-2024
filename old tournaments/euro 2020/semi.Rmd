---
title: "EURO 2020 predictions: semi-finals"
...
# Groupstage predictions: semi-finals (6-7 July)
...
exp_table <- cbind(euro_data_test_semi$home_team_name, euro_data_test_semi$away_team_name, exp_h, exp_a )
...
# Euro 2020 winning probabilities

We compute the final winning probabilities for each team. We simulated in advance the two semi-finals and the final and we report the final winning probabilities expressed in percentages (%).


```{r winning simulation, echo =FALSE}
...
# first semifinal
  ...
  final_team_1[y_prev[,1,1] == y_prev[,1,2]] <- sample(
                         ...
                         replace = TRUE,
                         c(foot_prob(fit4, euro_data)$prob_table[1,3],
                           foot_prob(fit4, euro_data)$prob_table[1,4]+
                             foot_prob(fit4, euro_data)$prob_table[1,5]))
...
# second semifinal
  ...
  final_team_2[y_prev[,2,1] == y_prev[,2,2]] <- sample(
    ...
    replace = TRUE,
    c(foot_prob(fit4, euro_data)$prob_table[2,3],
      foot_prob(fit4, euro_data)$prob_table[2,4]+
        foot_prob(fit4, euro_data)$prob_table[2,5]))
...
for (s in 1:dim(theta_home_prev)[1]){
  ...
  knitr::kable(final_probabilities)
```
