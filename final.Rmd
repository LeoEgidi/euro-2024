---
title: "EURO 2024 knockout predictions: Final"
...
# Knockout predictions: Final (14th July)
...
```{r final, echo = FALSE, out.width="80%", fig.width=11, eval = TRUE, fig.height=16,warning=FALSE, message=FALSE}
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# Libraries 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
library(tidyverse)
...
euro_data_test<- data.frame(
  date = rep(length(levels(times))+7, ngames_prev),
  
  home_team = c("Spain","France"),
  
  away_team = c("England","Netherlands"),
...
fit <- stan_foot(data = euro_data_stan[,-6],
                 ...
                 ind_home = "FALSE")
save(fit, file = paste0(MODELS_PATH,"final_fit.rds"))
#load(paste0(MODELS_PATH,"final_fit.rds"))
pars=extract(fit)
save(pars, file = paste0(MODELS_PATH,"final_pars.RData"))

prob <- foot_prob(fit, euro_data_stan[,-6],"Spain","England")
colnames(prob$prob_table) <- c("home", "away",
                               "home win", "draw", "away win", "mlo")
knitr::kable(prob$prob_table)
write.csv(prob$prob_table, paste0(TABLES_PATH,"final.csv"), row.names = FALSE)
prob$prob_plot
```
